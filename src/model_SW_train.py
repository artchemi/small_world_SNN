...
os.environ['CYTHON_CACHE_DIR'] = '/tmp/cython'
# Устанавливаем numpy как цель для codegen
...
#     for connName in save_conns:
#         conn = connections[connName]
#         # Extract weights as numpy array before saving
#         weight_values = np.zeros(len(conn.source), len(conn.target))
#         weight_values[conn.i, conn.j] = conn.w[:]  # Get the weight values
#         np.save(data_path + 'weights/' + connName + ending, weight_values)

def save_connections(ending = ''):
    print('save connections')
    for connName in save_conns:
        conn = connections[connName]
        # Исправлено создание массива - передаем кортеж размеров
        weight_values = np.zeros((len(conn.source), len(conn.target)))
...
def save_theta(ending = ''):
    ...
    for pop_name in population_names:
        np.save(data_path + 'weights/theta_dir/theta_' + pop_name + ending, neuron_groups[pop_name + 'e'].theta)
...
if num_examples <= 60000:
    save_connections_interval = 1000  # 10000
else:
...
for subgroup_n, name in enumerate(population_names):
    ...
    if test_mode or weight_path[-8:] == 'weights/':
        ...
        neuron_groups['e'].theta = np.load(weight_path + 'XeAe_dir/XeAe_10000' '.npy') * b2.volt
    else:
...
# TODO: в input_connection_names надо добавить малый мир
for name in input_connection_names:
    print('create connections between', name[0], 'and', name[1])
    for connType in input_conn_names:
        connName = name[0] + connType[0] + name[1] + connType[1]
        weightMatrix = get_matrix_from_file(weight_path + connName + ending + '.npy')
        # TODO: добавить загрузку матрицы весов с малого мира
...
j_arr = np.asarray(G.edges()).T[0]
i_arr = np.asarray(G.edges()).T[1]

connections['small_world'] = b2.Synapses(neuron_groups['e'], neuron_groups['e'], model=model, on_pre=pre, on_post=post)
connections['small_world'].connect(j=j_arr, i=i_arr)
# connections['small_world'].w = 'rand()*wmax_ee'
connections['small_world'].w = np.random.random((1, len(G.edges())))
# connections['small_world'].w = np.load('weights/small_world/AeSW_10000.npy')

# connections['small_world'].delay = 'minDelay + rand() * deltaDelay'

# connections['small_world_i'] = b2.Synapses(neuron_groups['i'], neuron_groups['i'], model=model, on_pre=pre, on_post=post)
# connections['small_world_i'].connect(j=j_arr, i=i_arr)
# connections['small_world_i'].w = 'rand()*wmax_ee'

print(connections['XeAe'].w)
print(connections['small_world'].w)

def plot_weights_dynamics(weights_xeae: list, weights_sw: list) -> None:
    pass
...
#     print('run number:', j+1, 'of', int(num_examples))
    ...
    print(np.sum(connections['small_world'].w))
    print(connections['XeAe'].w)

    # Сохраняем веса перед показом картинки
    if (not(test_mode) and j % 1000 == 0) or j == 0:
        with open(f'weights/small_world/AeSW_{j}.npy', 'wb') as file:
            np.save(file, connections['small_world'].w)

        # with open(f'weights/XeAe_dir/XeAe_{j}.npy', 'wb') as file:
        #     np.save(file, connections['XeAe'].w)
    ...
    if np.sum(current_spike_count) < 5:
        ...
        # Accuracy
        if j % accuracy_update_interval == 0 and j > 0:
            current_accuracy = np.mean(performance[:int(j/update_interval)])
            print(f'Accuracy after {j} images: {current_accuracy:.2f}%')
